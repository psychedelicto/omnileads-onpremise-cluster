...
omnileads_release=oml-113-issue-cambio-repositorio-postgres-rompe-install
ami_user=omnileads
...
echo "******************** yum update and install paq ***************************"
yum update --exclude=glibc* -y && yum install git nfs-utils python3-pip python3-devel kernel-devel -y
