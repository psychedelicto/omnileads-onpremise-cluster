...
echo "************************ Set Network config variables *************************"
export IPADDR_MASK=$(ip addr show $NIC | grep "192.168" | awk '{print $2}')
export NETADDR_IPV4=$(ipcalc -n $IPADDR_MASK |cut -d = -f 2)
export NETMASK_PREFIX=$(ip addr show $NIC | grep "192.168" | awk '{print $2}' | cut -d/ -f2)

echo "$NETADDR_IPV4 - $IPADDR_MASK - $NETMASK_PREFIX"
sleep 10
